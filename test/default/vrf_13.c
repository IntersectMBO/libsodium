...
int main(void)
{
    ...
    assert(crypto_vrf_ietfdraft13_BYTES == 80);
    ...
    for (i = 0U; i < (sizeof test_data) / (sizeof test_data[0]); i++) {
        ...
        if (crypto_vrf_ietfdraft13_prove(proof, sk, messages[i], i) != 0){
            printf("crypto_vrf_prove() error: [%u]\n", i);
        }
        printf("%s\n", sodium_bin2hex(proof_hex, sizeof proof_hex, proof, sizeof proof));

        if (crypto_vrf_ietfdraft13_verify(output, pk, proof, messages[i], i) != 0){
        ...
        if (i > 0) {
            if (crypto_vrf_ietfdraft13_verify(output, pk, proof, messages[i], i-1) == 0){
                printf("verify succeeded with truncated message: [%u]\n", i);
            }
        }

        if (crypto_vrf_ietfdraft13_proof_to_hash(output, proof) != 0){
