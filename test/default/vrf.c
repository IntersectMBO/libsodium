...
int main(void)
{
    ...
    seed            = (unsigned char *) sodium_malloc(crypto_vrf_SEEDBYTES);
    ...
    for (i = 0U; i < (sizeof test_data) / (sizeof test_data[0]); i++) {
        ...
        if (crypto_vrf_verify(output, pk, proof, messages[i], i) != 0){
        ...
        if (i > 0) {
            if (crypto_vrf_verify(output, pk, proof, messages[i], i-1) == 0){
